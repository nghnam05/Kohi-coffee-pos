  ...
  return (
    <>
          ...
          {isOpen && (
            ...
            >
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-xs transition-all cursor-pointer text-left ${
                  isDark
                    ? 'bg-sky-500/10 text-[#0284c7] dark:text-[#38BDF8] font-semibold'
                    : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/70 font-medium'
                }`}
              >
                <div className="flex items-center gap-2">
      ...
      <div
        role="radiogroup"
        aria-label="Chuyển đổi giao diện Sáng / Tối"
        className={`hidden sm:inline-flex items-center p-1 rounded-xl bg-slate-200/70 dark:bg-slate-900/80 border border-slate-300/70 dark:border-white/10 shadow-inner select-none shrink-0 ${className}`}
      >
        {/* Light mode option */}
        <button
          type="button"
