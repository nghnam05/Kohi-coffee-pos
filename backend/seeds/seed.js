...
async function runSeed() {
  ...
  // 9 Days distribution weights (01/09 - 09/09/2026):
  const dayWeights = [
    1.0,  // Day 1 (Tue)
    1.0,  // Day 2 (Wed)
    1.05, // Day 3 (Thu)
  ...
  const daysPool = Array.from({ length: 9 }, () => []);
  ...
  console.log('\n--- 4. Generating Orders, Payments, and Reviews (01/09 - 09/09) ---');
  const allOrders = [];
  const allPayments = [];
  const allReviews = [];
  ...
  console.log('\n--- 7. Generating Full Staff Attendance (All 3 Shifts x 9 Days) ---');
