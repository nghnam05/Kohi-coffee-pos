...
describe('CouponsService', () => {
  ...
  describe('generateRewardCouponForOrder', () => {
    it('should create an auto-generated 10% coupon with prefix KOHI10-', async () => {
      function MockCouponModel(this: any, dto: any) {
        Object.assign(this, dto);
        this.save = jest.fn().mockResolvedValue({ _id: 'new_reward_id', ...dto });
      }
      (service as any).couponModel = MockCouponModel;

      const coupon = await service.generateRewardCouponForOrder('order_123', 350000);
      expect(coupon.code).toMatch(/^KOHI10-[A-Z0-9]{6}$/);
      expect(coupon.type).toBe('percent');
      expect(coupon.value).toBe(10);
      expect(coupon.isAutoGenerated).toBe(true);
      expect(coupon.rewardedFromOrderId).toBe('order_123');
    });
  });
});
