...
export class CouponsService implements OnModuleInit {
  ...
  /**
   * Tự động tạo mã voucher 10% khi hóa đơn thanh toán trên 300.000đ
   */
  async generateRewardCouponForOrder(orderId: string, orderTotal: number): Promise<CouponDocument> {
    const randomCode = Math.random().toString(36).substring(2, 8).toUpperCase();
    const code = `KOHI10-${randomCode}`;
